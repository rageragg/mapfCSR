create or replace package body json_ext as
  ...
  */
  ...
  function is_integer(v tron2000.json_value_web) return boolean as
  ...
  function to_json_value(d date) return tron2000.json_value_web as
  begin
    return tron2000.json_value_web(to_char(d, format_string));
  end;

  --notice that a date type in json is also a varchar2
  function is_date(v tron2000.json_value_web) return boolean as
    temp date;
  begin
  ...
  function to_date2(v tron2000.json_value_web) return date as
  begin
    if(v.is_string) then
  ...
  --JSON Path getters
  function get_json_value(obj json, v_path varchar2, base number default 1) return tron2000.json_value_web as
    path json_list;
  ...
  procedure put_internal(obj in out nocopy json, v_path varchar2, elem tron2000.json_value_web, base number) as
    val tron2000.json_value_web := elem;
    path json_list;
    backreference json_list := json_list();

    keyval tron2000.json_value_web; keynum number; keystring varchar2(4000);
    temp tron2000.json_value_web := obj.to_json_value;
    obj_temp  json;
    list_temp json_list;
    inserter tron2000.json_value_web;
  begin
        ...
        if(temp.is_object()) then
          ...
          if(list_temp.count < keynum) then
            ...
            for i in list_temp.count+1 .. keynum loop
              list_temp.append(TRON2000.json_value_web.makenull);
            end loop;
            backreference.remove_last;
  ...
  --    inserter.print(false);
      if( i = 1 ) then
        ...
        if(keyval.is_string()) then
          ...
          keynum := keyval.get_number();
          declare
            t1 tron2000.json_value_web := obj.get(keynum);
          begin
        ...
        if(temp.is_object()) then
          ...
          if(keyval.is_string()) then
            ...
            keynum := keyval.get_number();
            declare
              t1 tron2000.json_value_web := obj_temp.get(keynum);
            begin
              keystring := t1.mapname;
            end;
          end if;
  ...
  begin
    put_internal(obj, path, tron2000.json_value_web(elem), base);
  end;
  ...
  procedure put(obj in out nocopy json, path varchar2, elem tron2000.json_value_web, base number default 1) as
  begin
    if(elem is null) then raise_application_error(-20110, 'Cannot put null-value'); end if;
...
--    end if;
  ...
  function pp(obj json, v_path varchar2) return varchar2 as
    json_part tron2000.json_value_web;
  begin
...
--    dbms_output.put_line('L B'||v_amount);
  ...
  function encode(binarydata blob) return tron2000.json_value_web as
    obj tron2000.json_value_web;
    c clob := empty_clob();
    benc blob;
...
--      dbms_output.put_line('Finshed base64 encoding.');
  ...
  function decode(v tron2000.json_value_web) return blob as
    c clob := empty_clob();
    b blob := empty_blob();
    bret blob;
