create or replace package body dc_k_crea_terceros_web_mcr is
  ...
  */ -----------------------------------------------------------
  ...
  BEGIN
    --
    ptraza('pl_crea_terceros', 'a', 'Inicio en pl_guarda_datos_tercero ');
    ...
    IF g_table_key_values.COUNT > 0 THEN
      ...
      END LOOP;
      --
      ptraza('pl_crea_terceros', 'a', 'Final pl_guarda_datos_tercero ');
      ptraza('pl_crea_terceros', 'a', '------');
      --  
    END IF;
    --
  ...
  BEGIN
    --
    ptraza('pl_crea_terceros', 'w', 'INICIO: '|| g_fec_actu ||' en pl_crea_terceros' );
    ptraza('pl_crea_terceros',
           'a',
    ...
    IF p_dat_asegurado.COUNT > 0 THEN
      ...
      FOR l_fila_aseg IN p_dat_asegurado.FIRST .. p_dat_asegurado.LAST LOOP
        --
        ptraza('pl_crea_terceros', 'a', 'Procesando JSON');
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ptraza('pl_crea_terceros', 'a', 'Error general en pl_crea_terceros ' || sqlerrm);
