...
*/
  ...
  function getMemName(mem tron2000.json_value_web, spaces boolean) return varchar2 as
  begin
    if(spaces) then
...
--    dbms_lob.append(buf_lob, buf_str);
  ...
  procedure ppMem(mem tron2000.json_value_web, indent number, buf in out nocopy clob, spaces boolean, buf_str in out nocopy varchar2) as
    numbuf varchar2(4000);
  begin
...
--            dbms_output.put_line('VSTR_SIZE:'||length(v_str));
  ...
  procedure pretty_print_any(json_part tron2000.json_value_web, spaces boolean default true, buf in out nocopy clob, line_length number default 0, erase_clob boolean default true) as
    buf_str varchar2(32767) := '';
    numbuf varchar2(4000);
    amount number := dbms_lob.getlength(buf);
  begin
...
/* Varchar2 method start here */
  ...
  procedure ppMem(mem tron2000.json_value_web, indent number, buf in out nocopy varchar2, spaces boolean) as
    str varchar2(400) := '';
  begin
  ...
  function pretty_print_any(json_part tron2000.json_value_web, spaces boolean default true, line_length number default 0) return varchar2 as
    buf varchar2(32767) := '';
  begin
    case json_part.get_type
      when 'number' then
