create or replace PACKAGE BODY em_k_a1000802_trn AS
 ...
 || * Se modifica el proceso p_inserta para admina los nuevos campos NO NULLABLE
 */ --------------------------------------------------------
 --
 ...
 */ --------------------------------------------------
 --
 PROCEDURE p_inserta( p_reg IN OUT a1000802%ROWTYPE )
 IS
 --
 BEGIN
   --
   IF p_reg.mca_domicilio_comprobado IS NULL THEN
      p_reg.mca_domicilio_comprobado := 'N';
   END IF;

   IF p_reg.mca_tlf_numero_comprobado IS NULL THEN
      p_reg.mca_tlf_numero_comprobado := 'N';
   END IF;

   IF p_reg.mca_fax_numero_comprobado IS NULL THEN
      p_reg.mca_fax_numero_comprobado := 'N';
   END IF;

   IF p_reg.mca_email_comprobado IS NULL THEN
      p_reg.mca_email_comprobado := 'N';
   END IF;

   IF p_reg.mca_domicilio_com_comprobado IS NULL THEN
      p_reg.mca_domicilio_com_comprobado := 'N';
   END IF;

   IF p_reg.mca_domicilio_etiq_comprobado IS NULL THEN
      p_reg.mca_domicilio_etiq_comprobado := 'N';
   END IF;

   IF p_reg.mca_tlf_numero_com_comprobado IS NULL THEN
      p_reg.mca_tlf_numero_com_comprobado := 'N';
   END IF;

   IF p_reg.mca_fax_numero_com_comprobado IS NULL THEN
      p_reg.mca_fax_numero_com_comprobado := 'N';
   END IF;

   IF p_reg.mca_email_com_comprobado IS NULL THEN
      p_reg.mca_email_com_comprobado := 'N';
   END IF;

   IF p_reg.mca_tlf_movil_comprobado IS NULL THEN
      p_reg.mca_tlf_movil_comprobado := 'N';
   END IF;

   IF p_reg.mca_busca_comprobado IS NULL THEN
      p_reg.mca_busca_comprobado := 'N';
   END IF;

   IF p_reg.mca_txt_email_comprobado IS NULL THEN
      p_reg.mca_txt_email_comprobado := 'N';
   END IF;
   --
   INSERT INTO a1000802(cod_cia                  ,
                        ...
                        mca_domicilio_comprobado,
                        mca_tlf_numero_comprobado,
                        mca_fax_numero_comprobado,
                        mca_email_comprobado,
                        mca_domicilio_com_comprobado,
                        mca_domicilio_etiq_comprobado,
                        mca_tlf_numero_com_comprobado,
                        mca_fax_numero_com_comprobado,
                        mca_email_com_comprobado,
                        mca_tlf_movil_comprobado,
                        mca_busca_comprobado,
                        mca_txt_email_comprobado
         )
         VALUES(  p_reg.cod_cia                  ,
                  ...
                  p_reg.mca_domicilio_comprobado,
                  p_reg.mca_tlf_numero_comprobado,
                  p_reg.mca_fax_numero_comprobado,
                  p_reg.mca_email_comprobado,
                  p_reg.mca_domicilio_com_comprobado,
                  p_reg.mca_domicilio_etiq_comprobado,
                  p_reg.mca_tlf_numero_com_comprobado,
                  p_reg.mca_fax_numero_com_comprobado,
                  p_reg.mca_email_com_comprobado,
                  p_reg.mca_tlf_movil_comprobado,
                  p_reg.mca_busca_comprobado,
                  p_reg.mca_txt_email_comprobado
               );
    --
 END p_inserta;
 --
