...
AS
	...
	*/ -------------------------------------------------------------------
	...
	PROCEDURE p_proceso (p_cod_cia          a2990700.cod_cia           %TYPE,
		...
		CURSOR c_a2990700 (p_cod_cia    a2990700.cod_cia         %TYPE
			...
			AND fec_efec_recibo BETWEEN p_fec_desde AND P_fec_hasta
			GROUP BY num_recibo
		...
		--
		CURSOR c_x5020039_todo (pi_num_recibo x5020039.num_recibo %TYPE) IS
			SELECT *
			FROM x5020039
			WHERE cod_cia    = p_cod_cia
		...
		BEGIN
			--
			IF g_cod_mensaje BETWEEN 20000 AND 20999 THEN
				--
		...
		--
		-- determina el pagador 
		FUNCTION fp_pagador( p_num_poliza IN a2000060.num_poliza%TYPE ) RETURN BOOLEAN IS 
			-- -----------------------------
			-- Obtiene el pagador de la poliza si existiera
			-- -----------------------------
			CURSOR c_pagador IS
		...
		BEGIN 
			--
			OPEN c_pagador;
			FETCH c_pagador INTO l_tip_docum, l_cod_docum;
			l_ok := c_pagador%FOUND;
			CLOSE c_pagador;
			--
			RETURN l_ok;
			--
	...
	BEGIN
		...
		--
		IF NVL(trn_k_global.ref_f_global('MARCAR_CT'),'N') = 'S' AND p_cod_cta_simp IS NOT NULL AND p_fec_cobro IS NOT NULL	THEN
			--
			...
			--
			IF p_cod_mon IS NOT NULL THEN
				--
				-- La moneda de cobro es != a la moneda del recibo
				IF l_cod_mon_cobro  != p_cod_mon THEN
					--
					IF l_cod_mon_cobro != g_cod_mon_cia THEN
		...
		--
		-- Version 1.08
		IF trn_k_global.ref_f_global('num_aviso') IS NULL THEN
			...
			BEGIN
				...
				BEGIN
					...
					EXCEPTION
						WHEN OTHERS
							THEN 
								g_clave := ' a2000030 :[' ||TO_CHAR(p_cod_cia)||'-'||
											gc_k_a2990700.f_num_poliza||'-'||
				...
				END;
				--
				IF NVL(em_k_a2000030.f_mca_provisional,'N') = 'N' THEN
					--
					...
					IF NOT fp_pagador( gc_k_a2990700.f_num_poliza  ) THEN
					...
					BEGIN
						...
						dc_k_a1002201.p_lee(p_cod_cia          => p_cod_cia                       ,
											...
											p_num_secu_cta_tar => em_k_a2000030.f_num_secu_cta_tar);
						--
						IF dc_k_a1002201.f_mca_inh = trn.SI THEN
							--
						...
						--
						EXCEPTION
							WHEN l_cta_inh_exception THEN
							--
							RAISE_APPLICATION_ERROR(-20000, 
									ss_k_mensaje.f_texto_v(51001015, TO_CHAR(gc_k_a2990700.f_num_poliza )  || ' ' ||
							...
							--
							WHEN OTHERS	THEN 
								g_clave := ' a1001331 :[' ||to_char(p_cod_cia)
					...
					IF l_tip_clase_gestor = '4' THEN
						...
						l_num_tarjeta := NULL;
						--
					ELSIF l_tip_clase_gestor = '8' THEN
						-- 'TA'
					...
					ELSE
						...
						--
						IF NVL(trn_k_global.ref_f_global('MARCAR_CT'),'N') = 'S' 
							AND p_cod_cta_simp IS NOT NULL 
							AND p_fec_cobro IS NOT NULL
							AND l_cod_mon_cobro IS NOT NULL THEN
							--
							IF p_cod_mon IS NULL THEN
								--
								IF l_cod_mon_cobro = gc_k_a2990700.f_cod_mon THEN
									l_val_cambio_cobro := 1;
								ELSE
									-- La moneda de cobro es la del pais y la de los recibos es extranjera.
							...
							ELSE
								...
								--
								l_imp_cobro := dc_k_a1000400.f_redondea_importe( 	g_cod_mon_cia,
																					dc_k_oper_monedas.f_a_pais(
																						gc_k_a2990700.f_tot_recibo,
																						l_val_cambio_cobro
																					)
																				);
								--
					...
					--
					--
					OPEN   c_bloquea_recibo( p_cod_cia, reg_700.num_recibo);
					...
					gc_p_remesa_recibo_act_tip_sit(p_cod_cia, reg_700.num_recibo);
					--
				END IF;
				--
			END;
	...
	*/ --------------------------------------------------------
	...
	|| Creaci?n del Package
	*/ --------------------------------------------------------
	PROCEDURE p_borra_rec_x5020039( p_cod_cia     x5020039.cod_cia      %TYPE,
	...
	|| Creaci?n del Package
	*/ --------------------------------------------------------
	PROCEDURE p_inserta_x5020039( p_cod_cia      x5020039.cod_cia      %TYPE,
								  p_num_recibo   x5020039.num_recibo   %TYPE,
	...
	BEGIN
		--
		INSERT INTO X5020039( cod_cia, num_recibo, imp_recibo, 
		                      cod_mon, cod_cta_simp,
			                  fec_actu, cod_usr
							)
			VALUES( p_cod_cia, p_num_recibo, p_imp_recibo, 
					p_cod_mon, p_cod_cta_simp,
